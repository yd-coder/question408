							...
							<view class="topic-content">
								...
								<view class="answer-sheet">
									<view class="item h-flex-x" v-for="(sheetItem,sheetIndex) in item.answerSheet"
										:key="sheetIndex" @tap="onAnswerSheet(sheetIndex,item._id)">
	...
	export default {
		...
		methods: {
			...
			onAnswerSheet(index, id) {
				...
				// 上传题目id，用户openid到云函数增加刷题数(因阿里云免费额度限制，调用云函数请求频繁超时，暂不使用云函数)
				const openid = JSON.parse(uni.getStorageSync('userInfo')).openid
				uniCloud.database().collection('user').where({
					openid: openid
				}).get().then(res => {
					if (!res.result.data[0].questionList.includes(id)) {
						let newArr = res.result.data[0].questionList.concat(id)
						uniCloud.database().collection('user').where({
							openid: openid
						}).update({
							questionList: newArr,
							count: res.result.data[0].count + 1
						})
					}
				})
			...
			addFavorite() {
				if (this.isFavorite) {
					const openid = JSON.parse(uni.getStorageSync('userInfo')).openid
					let newItem = JSON.parse(JSON.stringify(this.favorite.filter(obj => obj._id !== this.dataList[this
						.topicIndex]._id)))
					uniCloud.database().collection('user').where({
						openid: openid
					}).update({
					...
					favorite.push(this.dataList[this.topicIndex])
					uniCloud.database().collection('user').where({
						openid: openid
					}).update({
						favorite: favorite
